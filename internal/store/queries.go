...
`
 ...
 `
	...
	createIDNameIndex = `CREATE UNIQUE INDEX idx_unique_user_id_name ON public.Passwords (user_id, name);
`
	...
	getUserSavedPassword = `SELECT name, data FROM "Passwords" WHERE name = $1 AND user_id = $2`
)
